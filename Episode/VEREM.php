...
<body>
	...
	<h2>View Program Log</h2>
        ...
        <tr>
            <table colspan="7" width="1350" valign="top" style="background-color:white;">
               ...
               <tr> <!-- Row for displaying already entered data -->
                   <?php
                     $query = "select * from SONG where callsign='" . addslashes($PROGRAMARRAY['callsign']) . "' and programname='" . addslashes($PROGRAMARRAY['programname']) . "' and date='" . addslashes($PROGRAMARRAY['date']) . "' and starttime='" . $PROGRAMARRAY['starttime'] . "' order by time";
                     ...
                     else
                     {
                         while ($list=mysql_fetch_array($listed))
                         {
                           ...
                           $songlang = mysql_query("select languageid from LANGUAGE where callsign='" . $list['callsign'] . "' and programname='" . $list['programname'] . "' and date='" . $_POST['user_date'] . "' and starttime='" . $_POST['user_time'] . "' and songid='". $list['songid'] ."'");
        ...
        <td>
        <form name="edit" action="p3update.php" method="POST">
        ...
        <td>
        <form name="verify" action="p3advremove.php" method="POST">
        <input type="hidden" name="callsign" value=<?php echo "\"" . $PROGRAMARRAY['callsign'] . "\"" ?> />
            <input type="hidden" name="program" value=<?php echo "\"" . $_POST['program'] . "\"" ?> />
            <input type="hidden" name="date" value=<?php echo "\"" . $_POST['user_date'] . "\"" ?> />
            <input type="hidden" name="time" value=<?php echo "\"" . $_POST['user_time'] . "\"" ?> />
            <input type="submit" value="Verify Removal">
        </form>
        </td>
        <td colspan="100%">
