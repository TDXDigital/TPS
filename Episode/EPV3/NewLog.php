	...
	function urlencodephp (str) {
		...
		function getCallsign(value_name){
			...
			if(value_name==0){
				...
				{// code for IE6, IE5 (Not Supported)
	   			...
	   			xmlhttp.open("GET","../EPV2/AJAX/getcallsign.php?n="+urlencodephp(value_name),true);
	   			xmlhttp.send();
    	...
    	//if (datefield.type!="date"){ //if browser doesn't support input type="date", initialize date picker widget:
    		jQuery(function($){ //on document.ready
        		$('#airdate').datepicker({
        			dateFormat: "yy-mm-dd"
        		}); // for individual date boxes
        		$('#prdate').datepicker({
        			dateFormat: "yy-mm-dd"
        		});
    		})
		//}
		//if (datefield.type!="time"){ //if browser doesn't support input type="date", initialize date picker widget:
    		jQuery(function($){ //on document.ready
        		$('#airtime').timespinner(); // for individual date boxes
    		})
		//}
...
<body>
	...
	<div id="content">
		<form name="search" method="POST" action="AJAX/components/generate.php">
		<table border="0" class="tablecss">
			...
			<tr>
				<td>
					<select required title="Show Name" name="p" id="shownamebox" onchange="getCallsign(this.form.p.value)">
					<?php
			...
			//            $alias=$row["Alias"];
				...
				<td>
					<input name="prdate" type="text" id="prdate" disabled/>
					
				</td>
				<td>
					<input name="d" type="text" id="airdate" disabled value="<?php echo date('Y-m-d'); ?>"/>
				</td>
				<td>
					<input name="t" type="text" style="width:65px" id="airtime" disabled value="<?php echo date('h:00 a'); ?>"/>
				</td>
				<td>
