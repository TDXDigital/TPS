...
SOURCE DENON DN-700C WebControl
*/
...
Autocompleter.Base = Class.create({
    ...
    getTokenBounds: function() {
        ...
        for (var diff = arguments.callee.getFirstDifferencePos(value, this.oldElementValue), offset = diff == this.oldElementValue.length ? 1 : 0, prevTokenPos = -1, nextTokenPos = value.length, tp, index = 0, l = this.options.tokens.length; index < l; ++index) {
...
Autocompleter.Base.prototype.getTokenBounds.getFirstDifferencePos = function(newS, oldS) {
    for (var boundary = Math.min(newS.length, oldS.length), index = 0; index < boundary; ++index)
    ...
    setOptions: function(options) {
        this.options = Object.extend({
            ...
            selector: function(instance) {
                for (var ret = [], partial = [], entry = instance.getToken(), count = 0, i = 0; i < instance.options.array.length && ret.length < instance.options.choices; i++) {
                    var elem = instance.options.array[i],
                        foundPos = instance.options.ignoreCase ? elem.toLowerCase().indexOf(entry.toLowerCase()) : elem.indexOf(entry);
                    while (foundPos != -1) {
...
Ajax.InPlaceEditor = Class.create({
    ...
    checkForEscapeOrReturn: function(e) {
        ...
        else Event.KEY_RETURN == e.keyCode && this.handleFormSubmission(e)
    },
