...
if (!$con){
	...
	if(isset($_POST['changed'])){
			...
			if(!mysqli_query($con, $CHNA)){
				if(mysqli_errno($con)==1451)
				{
					...
					echo mysqli_errno($con) . "<br />". mysqli_error($con);
				}
			}
			else{
				$PROGNAME = addslashes($_POST['name']);
            ...
            foreach ($_POST['dj'] as $value) {
                if($value!='-1'){
                    ...
                    elseif(defined("DEBUG")?$DEBUG:false){
                        echo $dateup."<br>";
                    }
	...
	$result = mysqli_query($con, $SQLA) or die(mysqli_error());
	...
	$CALLS = "";
		...
		echo "</td>
				...
				$genop = "";
				while ($genrerow=mysqli_fetch_array($GENRES)) {
...
?>
	...
	<?php
		$sqlmaxpl = "select HitLimit, CCX, PLX, genre, Display_Order, Theme, displayorder from program where programname=\"" . $PROGNAME . "\" and callsign=\"". $CALLS . "\" ";
			if(!$limits = mysqli_fetch_array(mysqli_query($con, $sqlmaxpl))){
				echo mysqli_error();
			}
		$sqlgenq2 = "select * from genre where genreid=\"" . $limits['genre'] .  "\" ";
			if(!$GENREQ = mysqli_fetch_array(mysqli_query($con, $sqlgenq2))){
				echo mysqli_error();
			}
	 ?>
...
<tr><td colspan="100%">
	...
	<?php
		...
		if(!($perfres = $con->query($SQDJ))){
			echo mysqli_error();
		}
		else{
			//$alias=mysql_fetch_array($perfres);
			//echo $alias['Alias'];
			$size = mysqli_num_rows($perfres);
			echo "<td colspan=\"". $size . "\">";
			//echo $size;
			$djexc[] = NULL;
			$counter = 0;
            $inactive=FALSE;
			while($alias=mysqli_fetch_array($perfres)){
				...
				$TEMDJ = mysqli_query($con, $djsql);
				while ($djrow=mysqli_fetch_array($TEMDJ)) {
        			$Arc=$djrow["Alias"];
        			$name=$djrow["djname"];
