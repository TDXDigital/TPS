...
if (!$con){
	...
	echo 'ERROR! cannot obtain access... this terminal may not be authorised for access';
        ...
        $INSENDQU = "update episode set endtime='" . to24hour(addslashes($_POST['end'])) . "', totalspokentime='". addslashes($_POST['spoken'])."' WHERE callsign='".addslashes($_SESSION['callsign'])."' and programname='".addslashes($_SESSION['program'])."' and date='".addslashes($_SESSION['date'])."' ";

        mysql_query($INSENDQU,$con);
        $PROGRAMQUERY = "select * from episode where callsign='". addslashes($_SESSION['callsign'])."' and programname='" . addslashes($_SESSION['program']) . "' and date='". addslashes($_SESSION['date'])."' and starttime='".addslashes($_SESSION['time'])."'";
...
<body>
	...
	/*
        ...
        <td>
        <form name="edit" action="/legacy/oep/EPV3/logs.php" method="POST">
        ...
        <td>
        <form name="append" action="/legacy/oep/EPV3/logs.php" method="POST">
        <input type="text" hidden="true" name="callsign" value=<?php echo "\"" . $PROGRAMARRAY['callsign'] . "\"" ?> />
            <input type="text" hidden="true" name="program" value=<?php echo "\"" . $_POST['program'] . "\"" ?> />
            <input type="text" hidden="true" name="user_date" value=<?php echo "\"" . $_POST['user_date'] . "\"" ?> />
            <input type="text" hidden="true" name="user_time" value=<?php echo "\"" . $_POST['user_time'] . "\"" ?> />
