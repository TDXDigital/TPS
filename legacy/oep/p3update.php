...
if (!$con){
	...
	echo 'ERROR! cannot obtain access... this terminal may not be authorised for access';
		...
		$SQLProg = "SELECT `genre`.*, `program`.length from `genre`, `program` where 
`program`.programname=\"" . addslashes($_POST['program']) . "\" and 
`program`.callsign=\"" . addslashes($CALLSHOW) . "\" and `program`.genre=`genre`.genreid";
